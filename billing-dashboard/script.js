document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadSavedData() {
        ...
        if (saved) {
            try {
                billingData.monthlyData = JSON.parse(saved);
            } catch (e) {
    ...
    function loadMemberData(id) {
        ...
        const avgScore = Math.round((avgPerDayValue / 41) * 100);
    ...
    function getRankingInfo(avgPerDay) {
        if (avgPerDay >= 41) return { percentage: 100, label: 'EXCELLENT', color: 'var(--neon-green)' };
        if (avgPerDay >= 31) return { percentage: 80, label: 'VERY GOOD', color: '#c5e0b4' };
        if (avgPerDay >= 21) return { percentage: 70, label: 'GOOD', color: '#ffee44' };
        if (avgPerDay >= 10) return { percentage: 50, label: 'CRITICAL', color: 'var(--neon-red)' };
        return { percentage: 0, label: 'FAIL', color: '#555' };
    }
    ...
    function renderAnnualJobsView() {
        ...
        billingData.team.forEach(member => {
            ...
            html += `<td style="font-weight:bold;">${memberTotal}</td></tr>`;
    ...
    function handleImportCSV(e) {
        ...
        reader.onload = function (event) {
            ...
            for (let i = 1; i < lines.length; i++) {
                ...
                const target = parseInt(parts[5]) || 0;
                ...
                if (newMonthlyData[memberId] && monthIdx !== -1) {
                    newMonthlyData[memberId][monthIdx] = {
                        month: monthName,
                        actual, jobsActual, target, b31_60, b61_90, b90, quality, issues, holidays, workingDays
                    };
                }
            }
            ...
            alert("✓ Data updated successfully! The dashboard is now reflecting the new information.");
        };
